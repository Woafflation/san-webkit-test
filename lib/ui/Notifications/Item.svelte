...
>
  <div class="row v-center">
    <Svg id={TypeIcon[type] || type} class="icon-BjaWmC {type}" />
    <div class="title txt-m">{title}</div>
    <Svg id="close" class="btn close-1_HgNv" on:click={destroy} />
  </div>
  {#if description}
...
:global(.icon-BjaWmC.success) {
  fill: var(--green);
}
:global(.icon-BjaWmC.error) {
  fill: var(--red);
}
:global(.icon-BjaWmC.warning) {
  fill: var(--orange);
}
:global(.icon-BjaWmC.info) {
  fill: var(--blue);
}
